...
static int _curr_to_pmu_reg_idx(struct bcmpmu_accy_data *di, int curr)
{
	...
	if (acld_enabled) {
		...
		if (di->bcmpmu->charger_path & BCMPMU_INPUT_REGULATION) {
			for (i = (ARRAY_SIZE(bcmpmu_pmu_curr_acld_table) - 1); i >= 0; i--) {
				if (curr >= bcmpmu_pmu_curr_acld_table[i])
					return i;
			}			
		} else {					
...
int bcmpmu_accy_id_change_notify(uint8_t id)
 {
	...
	pr_accy(ERROR, "%s: gp_accy_data %x\n", __func__, gp_accy_data);
	...
	if (gp_accy_data->pmu_usb_id == PMU_USB_ID_GROUND)
		gp_accy_data->pmu_session_a = true;

	pr_accy(FLOW, "%s id %d\n", __func__, id);
	bcmpmu_accy_queue_event(gp_accy_data,
		PMU_ACCY_EVT_OUT_ID_CHANGE,
		&gp_accy_data->pmu_usb_id);
