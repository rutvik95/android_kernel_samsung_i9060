...
#define FW_VERSION 0x34 /*Config Version*/
#define BOOT_VERSION 0x05
#define CORE_VERSION 0x91
#define MAX_FW_PATH 255
#define TSP_FW_FILENAME "mms_ts.fw"
	...
	touch_is_pressed = 0;
		...
		if (tmp[0] & MMS_TOUCH_KEY_EVENT) {
			...
			printk("[Touchkey] code=%d, state=%d\n", key_code, key_state);
				...
				if(x < 0 || x > 480 || y < 0 || y > 800) // ػ󵵸 Ѿ�ǥ   TSP Reset
				{
					reset_mms_ts(info);
					return;
				}
		...
		if (info->finger_state[id] == 0) {
			info->finger_state[id] = 1;
//			printk("P [%2d],([%4d],[%3d]) w=%d, major=%d, minor=%d, angle=%d, palm=%d\n",
//				id, x, y, tmp[4], tmp[6], tmp[7], angle, palm);
			printk("P [%2d],[%3d],[%3d], w=%d, palm=%d\n",
				id, x, y, tmp[4], palm);
...
static irqreturn_t mms_ts_interrupt(int irq, void *dev_id)
{
	...
	if (sz == 0){
		msg[1].len = 8; // INT HIGH ø  Dummy Read
...
static void get_config_ver(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "I9060_ME_1202");
