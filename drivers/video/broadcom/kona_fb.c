...
#if defined(CONFIG_BACKLIGHT_RT8511B)
...
static int __ref kona_fb_probe(struct platform_device *pdev)
{
	...
	if(lcd_check==LCD_PANEL_NOT_CONNECTION){
...
#if !defined(CONFIG_BACKLIGHT_RT8511B)
	backlight_control(0xFF);
#else
	ql_backlight_i2c_write(0x164,0xFF,1);
#endif
...
//{{ Mark for GetLog
	...
	if (need_map_switch) {
		...
		if (uboot_kvirt) {
			...
			/* Clear uboot buffer to kernel's buff1 */
			pr_info("Claer frame buffer \n");
			memset(fb->fb.screen_base + (framesize / 2),
				0x00, framesize / 2);
			/* update display with 1:1 map */
			if (!fb->display_info->vmode)
				kona_clock_start(fb);
...
#endif
#if defined(CONFIG_MACH_HAWAII_SS_GOYA3G)
	g_display_enabled = 0;
#endif
	return 0;
...
struct regulator *lvds_31_reg;
struct regulator *lvds_12_reg;
#endif
