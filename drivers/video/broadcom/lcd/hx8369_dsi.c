...
struct hx8369_dsi_lcd *hx8369_lcd = NULL;
...
static void esd_test_timer_func(unsigned long data)
{
	...
	if (list_empty(&(hx8369_lcd->esd_work.entry))) {
		disable_irq_nosync(gpio_to_irq(hx8369_lcd->esd_port));
		queue_work(hx8369_lcd->esd_workqueue, &(hx8369_lcd->esd_work));
		pr_info("%s invoked\n", __func__);
	}

	mod_timer(&hx8369_lcd->esd_test_timer,  jiffies + (30*HZ));
}
#endif

void hx8369_esd_recovery(struct hx8369_dsi_lcd *lcd)
{
	pr_info("%s\n", __func__);	
...
static int hx8369_panel_probe(struct platform_device *pdev)
{
	...
	hx8369_lcd = kzalloc(sizeof(struct hx8369_dsi_lcd), GFP_KERNEL);
	if (!hx8369_lcd)
		return -ENOMEM;

	hx8369_lcd->dev = &pdev->dev;
	hx8369_lcd->bl = DEFAULT_GAMMA_LEVEL;
//	hx8369_lcd->lcd_id = gPanelID_hx8369;
	hx8369_lcd->current_brightness	= 255;
	hx8369_lcd->panel_awake = true;	
	
	dev_info(hx8369_lcd->dev, "%s function entered\n", __func__);			

//	dev_info(hx8369_lcd->dev, "panelID : [0x%02X], [0x%02X], [0x%02X]\n", gPanelID_hx8369[0], gPanelID_hx8369[1], gPanelID_hx8369[2]);		

	
	platform_set_drvdata(pdev, hx8369_lcd);	

	mutex_init(&hx8369_lcd->lock);
...
#ifdef ESD_OPERATION
	hx8369_lcd->esd_workqueue = create_singlethread_workqueue("esd_workqueue");
	if (!hx8369_lcd->esd_workqueue) {
		dev_info(hx8369_lcd->dev, "esd_workqueue create fail\n");
		return 0;
	}
	
	INIT_WORK(&(hx8369_lcd->esd_work), esd_work_func);
	
	hx8369_lcd->esd_port = ESD_PORT_NUM;
	
	if (request_threaded_irq(gpio_to_irq(hx8369_lcd->esd_port), NULL,
			esd_interrupt_handler, IRQF_TRIGGER_RISING, "esd_interrupt", hx8369_lcd)) {
			dev_info(hx8369_lcd->dev, "esd irq request fail\n");
			free_irq(gpio_to_irq(hx8369_lcd->esd_port), NULL);
			hx8369_lcd->lcd_connected = 0;
	}
	    
#ifdef ESD_TEST
	setup_timer(&hx8369_lcd->esd_test_timer, esd_test_timer_func, 0);
	mod_timer(&hx8369_lcd->esd_test_timer,  jiffies + (30*HZ));
#endif

	hx8369_lcd->esd_processing = false; 
	hx8369_lcd->lcd_connected = 1;	
	hx8369_lcd->esd_enable = 1;		
#endif	
	
#ifdef CONFIG_HAS_EARLYSUSPEND
  hx8369_lcd->earlysuspend.level   = EARLY_SUSPEND_LEVEL_DISABLE_FB - 1;
  hx8369_lcd->earlysuspend.suspend = hx8369_dsi_early_suspend;
  hx8369_lcd->earlysuspend.resume  = hx8369_dsi_late_resume;
  register_early_suspend(&hx8369_lcd->earlysuspend);
#endif	
