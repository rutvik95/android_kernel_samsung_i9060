...
struct sc7798_dsi_lcd *sc_7798_lcd = NULL;
...
static void esd_test_timer_func(unsigned long data)
{
	...
	if (list_empty(&(sc_7798_lcd->esd_work.entry))) {
		disable_irq_nosync(gpio_to_irq(sc_7798_lcd->esd_port));
		queue_work(sc_7798_lcd->esd_workqueue, &(sc_7798_lcd->esd_work));
		pr_info("%s invoked\n", __func__);
	}

	mod_timer(&sc_7798_lcd->esd_test_timer,  jiffies + (30*HZ));
}
#endif

void sc7798_esd_recovery(struct sc7798_dsi_lcd *lcd)
{
	pr_info("%s\n", __func__);	
...
static int sc7798_panel_probe(struct platform_device *pdev)
{
	...
	sc_7798_lcd = kzalloc(sizeof(struct sc7798_dsi_lcd), GFP_KERNEL);
	if (!sc_7798_lcd)
		return -ENOMEM;

	sc_7798_lcd->dev = &pdev->dev;
	sc_7798_lcd->bl = DEFAULT_GAMMA_LEVEL;
	sc_7798_lcd->current_brightness	= 255;
	sc_7798_lcd->panel_awake = true;	

	dev_info(sc_7798_lcd->dev, "%s function entered\n", __func__);			
	
	
	platform_set_drvdata(pdev, sc_7798_lcd);	

	mutex_init(&sc_7798_lcd->lock);
...
#ifdef ESD_OPERATION
	sc_7798_lcd->esd_workqueue = create_singlethread_workqueue("esd_workqueue");
	if (!sc_7798_lcd->esd_workqueue) {
		dev_info(sc_7798_lcd->dev, "esd_workqueue create fail\n");
		return 0;
	}
	
	INIT_WORK(&(sc_7798_lcd->esd_work), esd_work_func);
	
	sc_7798_lcd->esd_port = ESD_PORT_NUM;
	
	if (request_threaded_irq(gpio_to_irq(sc_7798_lcd->esd_port), NULL,
			esd_interrupt_handler, IRQF_TRIGGER_RISING, "esd_interrupt", sc_7798_lcd)) {
			dev_info(sc_7798_lcd->dev, "esd irq request fail\n");
			free_irq(gpio_to_irq(sc_7798_lcd->esd_port), NULL);
			sc_7798_lcd->lcd_connected = 0;
	}
	    
#ifdef ESD_TEST
	setup_timer(&sc_7798_lcd->esd_test_timer, esd_test_timer_func, 0);
	mod_timer(&sc_7798_lcd->esd_test_timer,  jiffies + (30*HZ));
#endif

	sc_7798_lcd->esd_processing = false; 
	sc_7798_lcd->lcd_connected = 1;	
	sc_7798_lcd->esd_enable = 1;		
#endif	
	
#ifdef CONFIG_HAS_EARLYSUSPEND
  sc_7798_lcd->earlysuspend.level   = EARLY_SUSPEND_LEVEL_DISABLE_FB - 1;
  sc_7798_lcd->earlysuspend.suspend = sc7798_dsi_early_suspend;
  sc_7798_lcd->earlysuspend.resume  = sc7798_dsi_late_resume;
  register_early_suspend(&sc_7798_lcd->earlysuspend);
#endif	
