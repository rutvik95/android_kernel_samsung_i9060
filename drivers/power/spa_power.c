...
static ssize_t ss_batt_ext_attrs_store(struct device *pdev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	switch(off)
	{
		...
		case SPA_POWER_PROP_SIOP_LEVEL:
			...
			value.intval = spa_power_iter->charger_info.charging_current * intval /100;
			if (value.intval > 0 && value.intval < spa_power_iter->pdata->charging_cur_usb)
				value.intval = spa_power_iter->pdata->charging_cur_usb;
