...
struct fsa9485_usbsw {
	...
	struct delayed_work		usb_delay_noty;
	int				dev1;
	int				dev2;
	int				mhl_removed;
	int				mansw;
...
#define SWITCH_ESC		2
#endif
static int fsa9485_write_reg(struct i2c_client *client, int reg, int data)
{
	int ret = 0;
	u8 buf[2];
...
static int fsa9485_read_reg(struct i2c_client *client, int reg, int *data)
{
	int ret = 0;
	int buf[1];
...
static int fsa9485_read_word_reg(struct i2c_client *client, int reg, int *data)
{
	int ret = 0;
    int buf[1];
	int data1,data2;   
...
static DEVICE_ATTR(switch, S_IRUGO | S_IWUSR, fsa9485_show_manualsw, fsa9485_set_manualsw);
...
void fsa9485_vbus_check(bool vbus_status)
{
	...
	schedule_delayed_work(&usbsw->vbus_work, msecs_to_jiffies(900));

}
EXPORT_SYMBOL(fsa9485_vbus_check);

static int fsa9485_read_vbus(void)
{
	struct i2c_client *client = local_usbsw->client;
...
static int fsa9485_detect_dev(struct fsa9485_usbsw *usbsw)
{
	...
	adc &= 0xff;
#endif
	dev_info(&client->dev, "%s: start dev1: 0x%x, dev2: 0x%x adc:0x%x\n",__func__, val1, val2, adc);
	...
	if (val1 || val2) {
		...
		if (val1 & DEV_USB ) {
			...
			dev_info(&client->dev, "JIG USB OFF or USB connect\n");
			if( fsa9485_read_vbus()){
				if(adc == 0x18){
					usbsw->adc = (int)adc;
					if (pdata->usb_cb){
						dev_info(&client->dev, "JIG USB OFF & USB connect\n");
						pdata->usb_cb(FSA9485_ATTACHED);
					}
				}
...
//ENABLE_OTG
			...
			if (pdata->jig_cb)
				pdata->jig_cb(FSA9485_ATTACHED, 0);
		...
		if (usbsw->dev1 & DEV_USB) {
			...
			if(usbsw->adc == 0x18){
				if (pdata->usb_cb){
					pr_info("%s DEV_JIG_USB_OFF or USB removed\n",__func__);
					pdata->usb_cb(FSA9485_DETACHED);
				}
			}
			else if (pdata->charger_cb)
...
//DISABLE_OTG
		...
		/* JIG */
			...
			if (pdata->jig_cb)
				pdata->jig_cb(FSA9485_DETACHED, 0);
			
			if(fsa9485_read_vbus()== 0)
				if (pdata->charger_cb)
					pdata->charger_cb(FSA9485_DETACHED);
		/* Desk Dock */
...
unsigned int musb_get_charger_type(void)
{
	...
	value2 = i2c_smbus_read_byte_data(client, FSA9485_REG_DEV_T2);
	
	printk("%s chrgr type1 0x%x\n", __func__, value);
	printk("%s chrgr type2 0x%x\n", __func__, value2);


	if(value & DEV_USB || value2 & DEV_T2_USB_MASK)
	{
		return POWER_SUPPLY_TYPE_USB;
	}
	else if(value & DEV_T1_CHARGER_MASK)
	{
		return POWER_SUPPLY_TYPE_USB_DCP;
	}
	else if(value & DEV_USB_CHG)
	{
...
static irqreturn_t fsa9485_irq_thread(int irq, void *data)
{
	...
	pr_info("fsa9485_irq_handler irq #: %x\n", irq);
#if 0
	...
	wake_lock_timeout(&(usbsw->muic_wake_lock), 1*HZ);

	cancel_delayed_work_sync(&usbsw->work);

	if(sleepstatus == true)
		schedule_delayed_work(&usbsw->work, msecs_to_jiffies(400));
	 else
...
static void fsa9485_vbus_work_cb(struct delayed_work *work)
{
	...
	dev_info(&usbsw->client->dev, "%s, vbus value %d\n", __func__, usbsw->vbus);
	...
	if(usbsw->vbus==0){
		...
		if( val1 || val2) {
			...
			//to avoid muic interrupt mask.
			fsa9485_read_word_reg(usbsw->client, FSA9485_REG_INT1,&intr);
			dev_info(&usbsw->client->dev,"%s muic intr err intr: 0x%x, dev1: 0x%x, dev2: 0x%x \n"
				,__func__, intr, val1, val2);
...
static void fsa9485_work_cb(struct work_struct *work)
{
	...
	struct i2c_client *client = usbsw->client;
	int intr, intr2, detect,temp, val1, val2;
	...
	fsa9485_read_reg(client, FSA9485_REG_DEV_T1,&val1);
	fsa9485_read_reg(client, FSA9485_REG_DEV_T2,&val2);
	dev_info(&client->dev,"%s first intr1: 0x%x, dev1: 0x%x, dev2: 0x%x \n"
		,__func__, intr, val1, val2);
...
static void usb_host_delay_work(struct delayed_work *work)
{
	struct fsa9485_usbsw *usbsw = local_usbsw;
	struct fsa9485_platform_data *pdata;
...
static int fsa9485_irq_init(struct fsa9485_usbsw *usbsw)
{
	...
	pr_info("%s\n", __func__);
	INIT_DELAYED_WORK(&usbsw->work, fsa9485_work_cb);
	INIT_DELAYED_WORK(&usbsw->vbus_work, fsa9485_vbus_work_cb);
	INIT_DELAYED_WORK(&usbsw->usb_delay_noty, usb_host_delay_work);
...
static void fsa9485_init_detect(struct work_struct *work)
{
	...
	if(usbsw->deskdock && fsa9485_read_vbus())
		if (usbsw->pdata->charger_cb)
			usbsw->pdata->charger_cb(FSA9485_ATTACHED);

	if(usbsw->audio_dock) {
		dev_info(&usbsw->client->dev,"usb audio is connected, usb module is not active yet. %s\n", __func__);
		schedule_delayed_work(&usbsw->usb_delay_noty, 10*HZ);
	}
}
			 ...
			 const struct i2c_device_id *id)
{
	...
	wake_lock_init(&(usbsw->muic_wake_lock), WAKE_LOCK_SUSPEND, "muic_wakelock");
...
fail1:
	...
	wake_lock_destroy(&(usbsw->muic_wake_lock));
...
static int fsa9485_suspend(struct i2c_client *client)
{
//	int value;
//	struct fsa9485_usbsw *usbsw = i2c_get_clientdata(client);
