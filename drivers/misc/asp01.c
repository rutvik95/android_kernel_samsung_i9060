...
static int asp01_init_code_set(struct asp01_data *data)
{
	...
	reg = i2c_smbus_read_byte_data(data->client, 0x10);
...
static void asp01_work_func(struct work_struct *work)
{
	...
	/* check usb jig & skip data cmd for factory test */
	if (!data->skip_data) {
	...
	struct device_attribute *attr, char *buf)
{
	...
	u16 mlt_cs_per[4] = {0,};
	u16 mlt_cr_per[4] = {0,};
	u16 line_up_cs_per[4];
	u16 line_up_cr_per[4];
	int count = 100;
	int err, i, j;
	struct asp01_data *data = dev_get_drvdata(dev);
	...
	for (i = 0; i < 4; i++) {
		line_up_cs_per[i] = mlt_cs_per[i];
		line_up_cr_per[i] = mlt_cr_per[i];
	}

	/* find 2nd minimum values */
	for (i = 0; i < 2; i++) {
		for (j = i + 1; j < 4; j++) {
			if (line_up_cs_per[i] > line_up_cs_per[j])
				swap(line_up_cs_per[i], line_up_cs_per[j]);
			if (line_up_cr_per[i] > line_up_cr_per[j])
				swap(line_up_cr_per[i], line_up_cr_per[j]);
		}
	}
...
#if 0
static void asp01_apply_3v3_set(struct asp01_data *data)
{
	u8 reg;

	reg = i2c_smbus_read_byte_data(data->client, REG_HW_CON4);
	init_reg[SET_HW_CON4][CMD] = (reg & 0xf0) | 0x09;
...
static int asp01_setup_irq(struct asp01_data *data)
{
	...
	if (NULL == data->pdata) {
		pr_err("[ASP01] %s: asp01 irq was not setting\n",
                    __func__);
		return rc;
	}

	rc = gpio_request(data->pdata->t_out, "RF_TOUCH_INT");
	if (rc < 0) {
		pr_err("[ASP01] %s: gpio %d request failed (%d)\n", 
                    __func__, data->pdata->t_out, rc);
		return rc;
	}
	printk(KERN_ERR "[ASP01] Request GPIO_%d Sucess!\n", data->pdata->t_out);

	rc = gpio_direction_input(data->pdata->t_out);
	if (rc < 0) {
		pr_err("[ASP01] %s: failed to set gpio %d as input (%d)\n",
                    __func__, data->pdata->t_out, rc);
		goto err_gpio_direction_input;
	}

	data->pdata->irq = gpio_to_irq(data->pdata->t_out);
	if (data->pdata->irq > 0) {
		rc = request_threaded_irq(data->pdata->irq,
			NULL, asp01_irq_thread,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING,
			"asp01", data);
		pr_info("[ASP01] %s: irq = %d\n", __func__, data->pdata->irq);
	
		if (rc < 0) {
			pr_err("[ASP01] %s request_threaded_irq fail err=%d\n",
				__func__, rc);
			return rc;
		} else {
                    printk(KERN_INFO "[ASP01] request_irq success IRQ_NO:%d\n", data->pdata->irq);
		}
		       ...
		       const struct i2c_device_id *id)
{
	...
	if (pdata) {
        ...
        /*RF_TOUCH_EN*/
	if (gpio_request(pdata ->power_gpio, "RF_TOUCH_EN")) {
	...
	printk(KERN_INFO "[ASP01] gpio_get_value of %d is %d\n",pdata ->power_gpio,gpio_get_value(pdata ->power_gpio));
	...
	/* creating device for test & calibration */
        data->dev = device_create( sensors_class, NULL, 0, NULL, "grip_sensor");
	if (IS_ERR(data->dev)) {
