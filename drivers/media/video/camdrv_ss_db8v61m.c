...
/* individual configuration */
	...
	|| defined(CONFIG_MACH_HAWAII_SS_HEAT_REV01) || defined(CONFIG_MACH_JAVA_SS_BAFFINLITE_REV00)\
	|| defined(CONFIG_MACH_JAVA_SS_BAFFINLITE_REV02) || defined(CONFIG_MACH_JAVA_SS_VICTOR3G_REV00)\
...
#define COPY_FROM_60HZ_TABLE(TABLE_NAME, ANTI_BANDING_SETTING) \
	...
	COPY_FROM_60HZ_TABLE (db8v61m_fps_auto_normal_regs, 60hz);
	COPY_FROM_60HZ_TABLE (db8v61m_fps_auto_Dark_regs, 60hz);
...
#define IS_SAME_NUM_OF_ROWS(TABLE_NAME) \
	...
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_auto_normal_regs) ) return (-2);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_auto_Dark_regs) ) return (-3);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_5_regs) ) return (-4);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_7_regs) ) return (-5);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_10_regs) ) return (-6);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_15_regs) ) return (-7);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_20_regs) ) return (-8);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_25_regs) ) return (-9);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_fps_30_regs) ) return (-10);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_vt_mode_regs) ) return (-11);
	if ( !IS_SAME_NUM_OF_ROWS(db8v61m_init_regs_smart_stay) ) return (-12);
...
int camdrv_ss_db8v61m_set_preview_start(struct v4l2_subdev *sd)
{
	...
	{
			if (ARRAY_SIZE(db8v61m_fps_25_regs) != (int)NULL)
...
static float camdrv_ss_db8v61m_get_exposureTime(struct v4l2_subdev *sd)
{
    ...
    camdrv_ss_i2c_write_2_bytes(client, 0xFF, 0xD0);

    camdrv_ss_i2c_read_1_byte(client, 0x04, &read_chvalue1);
    camdrv_ss_i2c_read_1_byte(client, 0x05, &read_chvalue2);
    camdrv_ss_i2c_read_1_byte(client, 0x06, &read_chvalue3);
    camdrv_ss_i2c_read_1_byte(client, 0x07, &read_chvalue4);
    camdrv_ss_i2c_read_1_byte(client, 0x0A, &read_chvalue5);
    camdrv_ss_i2c_read_1_byte(client, 0x0B, &read_chvalue6);
    ...
    exposureTime = ( (read_value5<<8) |read_value6) * ( (read_value3<<8) |read_value4) +( (read_value1<<8)|read_value2);
...
static int camdrv_ss_db8v61m_get_iso_speed_rate(struct v4l2_subdev *sd)
{
    ...
    unsigned char read_chvalue1=0,read_chvalue2=0;
    float GainValue = 0;
    int isospeedrating = 100;
   // int rows_num_=0;

    camdrv_ss_i2c_write_2_bytes(client, 0xFF, 0x81);
    camdrv_ss_i2c_read_1_byte(client, 0xa5, &read_chvalue1);
    camdrv_ss_i2c_read_1_byte(client, 0xa6, &read_chvalue2);
    read_value1 = (unsigned short)read_chvalue1;
    read_value2 = (unsigned short)read_chvalue2;


    CAM_INFO_PRINTK("%s, read_value1 =%x , read_value2 =%x \n",__func__,read_value1,read_value2);

    GainValue =  ( (read_value1<<8) |read_value2) ;
	GainValue =GainValue/1024;

    CAM_INFO_PRINTK("%s, read_value1 =%d, GainValue = %d \n",__func__,read_value1, GainValue);

    if(GainValue <= 2)
    {
        isospeedrating = 50;
    }
    else if(GainValue <= 3.5)
    {
        isospeedrating = 100;
    }
    else if(GainValue <= 7)
	...
	struct v4l2_exif_sensor_info *exif_param)
{
	...
	strcpy(exif_param->maxLensAperture,	"2971/1000");
