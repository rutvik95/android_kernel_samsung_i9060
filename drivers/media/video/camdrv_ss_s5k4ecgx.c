...
|| defined(CONFIG_MACH_HAWAII_SS_HEAT_REV00) || defined(CONFIG_MACH_HAWAII_SS_HEAT_REV01) \
|| defined(CONFIG_MACH_JAVA_SS_BAFFINLITE_REV00)|| defined(CONFIG_MACH_JAVA_SS_BAFFINLITE_REV02)\
|| defined(CONFIG_MACH_JAVA_SS_VICTOR3G_REV00) || defined(CONFIG_MACH_JAVA_SS_EVAL_REV00)
...
#if defined (CONFIG_MACH_HAWAII_SS_CS02) || defined (CONFIG_MACH_JAVA_SS_BAFFINLITE)
  ...
  //NIKHIL 
		...
		///////////////////////////////////////////////////swsw
					if(state->af_mode == FOCUS_MODE_AUTO){
						 err = camdrv_ss_i2c_set_config_register(client,s5k4exgx_focus_mode_normal_regs_default, ARRAY_SIZE(s5k4exgx_focus_mode_normal_regs_default), "focus_mode_normal_regs_default");
			...
			if(err < 0)
			{
				...
				err = camdrv_ss_i2c_set_config_register(client,s5k4exgx_focus_mode_macro_regs_default, ARRAY_SIZE(s5k4exgx_focus_mode_macro_regs_default), "focus_mode_macro_regs_default");		
...
{		
		spin_lock_irqsave(&lock, flags);
			gpio_set_value(GPIO_CAM_FLASH_SET, 1); 
           		camdrv_ss_SKY81290_flash_write_data(5, 1);     /*Setting the safety timer threshold ST_TH(5) to 1(100mA)*/
		//	camdrv_ss_SKY81290_flash_write_data(3, 7); 	/* Setting the safety timer duration register STDUR(3) to 7 (1250msec)*/
           	//	camdrv_ss_SKY81290_flash_write_data(1, 0);     /* write 100%(0) to FEN/FCUR(1) */
			camdrv_ss_SKY81290_flash_write_data(4, 0); 	/* write disable(0) to low battery threshold LB_TH(4) */
			/* enable */
        		gpio_set_value(GPIO_CAM_FLASH_EN, 1);
        		//camdrv_ss_SKY81290_flash_write_data(1, 16); 
			spin_unlock_irqrestore(&lock, flags);
			}
			break;
		}
        ...
        case FLASH_CONTROL_HIGH_LEVEL:
		{
			...
			gpio_set_value(GPIO_CAM_FLASH_SET, 1); 
			camdrv_ss_SKY81290_flash_write_data(4, 0); 	/* write disable(0) to low battery threshold LB_TH(4) */
			camdrv_ss_SKY81290_flash_write_data(2,21);
			spin_unlock_irqrestore(&lock, flags);
			}
			break;
		}
        ...
        case FLASH_CONTROL_LOW_LEVEL:
			...
			gpio_set_value(GPIO_CAM_FLASH_SET, 1); 
			camdrv_ss_SKY81290_flash_write_data(4, 0); 	/* write disable(0) to low battery threshold LB_TH(4) */
			camdrv_ss_SKY81290_flash_write_data(2,27);
			spin_unlock_irqrestore(&lock, flags);
			break;
		}
...
//	sensor->get_light_condition   = camdrv_ss_s5k4ecgx_get_light_condition;
	...
	sensor->focus_mode_normal_regs_default = s5k4exgx_focus_mode_normal_regs_default;
	sensor->rows_num_focus_mode_normal_regs_default     = ARRAY_SIZE(s5k4exgx_focus_mode_normal_regs_default);
	sensor->focus_mode_macro_regs_default = s5k4exgx_focus_mode_macro_regs_default;
	sensor->rows_num_focus_mode_macro_regs_default     = ARRAY_SIZE(s5k4exgx_focus_mode_macro_regs_default);
  /* flicker */
