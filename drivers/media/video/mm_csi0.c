...
int mm_csi0_set_dig_phy(struct lane_timing *timing)
{
	...
	if (cam_state.mode == CSI1CCP2) {
		...
		BRCM_WRITE_REG_FIELD(base, CAM_CLT, CLT1, 0x1);    // Need to adjust for a sensor : Clock lane termination time
		BRCM_WRITE_REG_FIELD(base, CAM_CLT, CLT2,  0x0);  // Need to adjust for a sensor : Clock lane settle time
